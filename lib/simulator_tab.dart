...
class _SimulatorTabState extends State<SimulatorTab> {
  ...
  void _loadCode() {
    try {
      _simulator.loadCode(widget.codeContent);
  ...
  String _formatCodeForSimulator(String code) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      // Format as address and instruction
      final address = '0x${(i * 4).toRadixString(16).padLeft(8, '0')}';
      final instruction =
          cleanLine.startsWith('0x') ? cleanLine : '0x$cleanLine';

      formattedLines.add('$address $instruction');
    }
                              ...
                              children: [
                                Expanded(
                                  ...
                                  child: _buildMetricTile(
                                    'Current PC',
                                    '0x${currentPc.toRadixString(16).padLeft(8, '0')}',
                                  ),
                                ),
                                Expanded(
                                  flex: 3,
                                  child: _buildMetricTile(
                                      'Instruction Count',
                                      _isSimulatorInitialized
                                          ? '${_simulator.getInstructionCount()}'
                                          : '0'),
                                ),
                                Expanded(
                                  flex: 2,
  ...
  Widget _buildInstructionPanel() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final address = '0x${(index * 4).toRadixString(16).padLeft(8, '0')}';
