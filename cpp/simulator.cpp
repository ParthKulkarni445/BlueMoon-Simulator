...
struct RegisterFile
{
    ...
    void printRegs()
    {
        for (int i = 0; i < 32; i++)
        {
            appendToConsole("x" + to_string(i) + ": " + regs[i]);
        }
    }
};
...
struct ALU
{
    ...
    {
        int aVal = hex_to_dec(ra);
        int bVal = hex_to_dec(rb);
        appendToConsole("ra: " + ra + " rb: " + rb);
...
struct PMI
{
    ...
    void load(char type)
    {
        ...
        {
            MDR = mem.text[address + 3] + mem.text[address + 2] + mem.text[address + 1] + mem.text[address]; // Loading word as instruction is always 32 bits
            if (MDR == "") {
                appendToConsole("No instruction at this address.");
        ...
        else if (address < 268468224)
        {
            ...
            case 'b':
                ...
                break; // Loading byte
            case 'h':
                MDR = mem.static_data[address + 1] + mem.static_data[address];
                break; // Loading half word
            default:
                MDR = mem.static_data[address + 3] + mem.static_data[address + 2] + mem.static_data[address + 1] + mem.static_data[address];
        ...
        else
        {
            ...
            case 'b':
                ...
                break; // Loading byte
            case 'h':
                MDR = mem.dynamic_data[address + 1] + mem.dynamic_data[address];
                break; // Loading half word
            default:
                MDR = mem.dynamic_data[address + 3] + mem.dynamic_data[address + 2] + mem.dynamic_data[address + 1] + mem.dynamic_data[address];
                break; // Loading word by default
            }
            if (MDR == "")
                MDR = "00000000"; // If no data at this address, return 0
    ...
    void store(char type)
    {
        ...
        if (address < 268435456)
        {
            mem.text[address + 3] = MDR.substr(0, 2);
            mem.text[address + 2] = MDR.substr(2, 2);
            mem.text[address + 1] = MDR.substr(4, 2);
            mem.text[address] = MDR.substr(6, 2); // Storing word as instruction is always 32 bits
        }
        else if (address < 268468224)
        {
            // address-=10; // Adjusting address to start from 0
            ...
            case 'b':
                ...
                break; // Storing byte
            case 'h':
                mem.static_data[address + 1] = MDR.substr(4, 2);
                mem.static_data[address] = MDR.substr(6, 2);
                break; // Storing half word
            default:
                mem.static_data[address + 3] = MDR.substr(0, 2);
                mem.static_data[address + 2] = MDR.substr(2, 2);
                mem.static_data[address + 1] = MDR.substr(4, 2);
                mem.static_data[address] = MDR.substr(6, 2);
        ...
        else
        {
            ...
            case 'b':
                ...
                break; // Storing byte
            case 'h':
                mem.dynamic_data[address + 1] = MDR.substr(4, 2);
                mem.dynamic_data[address] = MDR.substr(6, 2);
                break; // Storing half word
            default:
                mem.dynamic_data[address + 3] = MDR.substr(0, 2);
                mem.dynamic_data[address + 2] = MDR.substr(2, 2);
                mem.dynamic_data[address + 1] = MDR.substr(4, 2);
                mem.dynamic_data[address] = MDR.substr(6, 2);
                break; // Storing word by default
            }
    ...
    string getMemoryContent(string segment, int startAddr, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (addr < 268435456 && segment == "text") {
                value = (*memSegment)[addr + 3] + (*memSegment)[addr + 2] + 
                        (*memSegment)[addr + 1] + (*memSegment)[addr];
            } else {
                value = (*memSegment)[addr + 3] + (*memSegment)[addr + 2] + 
                        (*memSegment)[addr + 1] + (*memSegment)[addr];
            }
            
            if (value.empty()) value = "00000000";
...
private:
    ...
    string getImmediate(const string &binaryInstr, const string &opcode)
    {
        ...
        { // J-type
            im_val = extractBits(binaryInstr, 0, 0) + extractBits(binaryInstr, 12, 19) +
                     extractBits(binaryInstr, 11, 11) + extractBits(binaryInstr, 1, 10) + "0"; // Bits 0, 12-19, 11, 1-10
        }
...
public:
    ...
    void decode()
    {
        ...
        appendToConsole(memory.MDR + " instruction type " + instr_type + " opcode " + opcode + 
    ...
    bool step()
    {
        ...
        try {
            ...
            else if (opcode == "0000011")
            {
                ...
                rb = f.imm;
                f.execute("add");
                appendToConsole(f.instr_type.back() + "it is a checker to see what it does");
                f.iag.compute_nextPC("", "", false, false);
            ...
            else if (opcode == "1100011")
            {
                ...
                if (rz == "00000001")
                    ...
                    f.iag.compute_nextPC("", "", false, false);
                appendToConsole("the new pc becomes " + f.iag.pc);
            }
            else if (opcode == "0110111")
            {
                // lui
            ...
            else if (opcode == "1100111")
            {
                ...
                appendToConsole("imm value is " + f.imm + " which is added to rs2 " + rb + " so final return address is " + rz + " and the next addr- " + temp + " is stored to " + ra);
...
public:
    ...
    void loadCode(const string& codeStr) {
        ...
        while (getline(ss, line)) {
            ...
            string addrStr = line.substr(0, pos);
            string instrStr = line.substr(pos + 1);
            
            // Clean up strings
            if (addrStr.find("0x") != string::npos)
                addrStr = addrStr.substr(addrStr.find("0x") + 2);
                
            if (instrStr.find("0x") != string::npos)
                instrStr = instrStr.substr(instrStr.find("0x") + 2);
...
// Main function (not used in WebAssembly)
int main() {
    return 0;
}